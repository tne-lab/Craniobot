...
class MyButton:
    ...
    """
    def __init__(self, surface, index, x, y, w, h,
                 text,fsize = 18,
...
class MyLabel:
    ...
    def __init__(self, surface,  x, y, text, fsize = 20):
        self.surface = surface
        self.x = x
        self.y = y
        #self.w = w
    ...
    def draw(self):
        ...
        # WRITE LABEL text
        msg_font = pygame.font.SysFont('arial', self.fsize,bold=True)
...
class InfoBox:
    ...
    def draw(self):
        ...
        lbl_in_font = my_font.render(self.label, True, txt_color) #(0,0,0) = black
        ...
        for chr in self.text:
            ...
            if chr == "\n":
                ...
                each_line = ""
        #print("lines_in_txt ",lines_in_txt, lines)    
        if lines_in_txt > 0: #NOT EMPTY BOX, No info_boxes
            my_font = pygame.font.SysFont('arial', fsize)
            

            if lines_in_txt == 1: # simple info box (txt centered)
                msg_in_font = my_font.render(self.text, True, (0,0,0))
                msgHt = msg_in_font.get_height()
                msgY =  msgHt - 2
                msgWd = msg_in_font.get_width()
                msgX = (self.rect.width - msgWd)/2 # Center in box
                surface.blit(msg_in_font, self.rect.move(msgX,  0))
...
class WARNINGBox:
    """
    """
    def __init__(self, surface,x, y, w, h, label,label_pos, text ,fsize = 14,surface_color = (255,255,255)):
        self.surface = surface
        self.label_pos = label_pos # 'TOP','LEFT','RIGHT', or 'BOTTOM'
...
class get_user_input:
    def __init__(self, surface,x, y, w, h, label,label_pos, text ,fsize = 16):
    ...
    def draw(self):
        ...
        # WRITE LABEL
        my_font = pygame.font.SysFont('arial', self.fsize,bold=True)
        lbl_in_font = my_font.render(self.label, True, txt_color)
        lblHt =lbl_in_font.get_height()
        lblWd = lbl_in_font.get_width()
        ...
        # WRITE TEXT
        my_font2 = pygame.font.SysFont('arial', self.fsize, bold=True)
        txt_in_font = my_font2.render(self.text, True, txt_color)
